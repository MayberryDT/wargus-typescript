...
for (const [label, text, fragments] of [
  ...
  ["orders attack action", ordersSource, [
    ...
    "sourceAttackTargetPathResult(world, unit, target)",
    'if (result.status === "unreachable")',
    "order.targetId = null",
    "findPathResult(world, unit, order.targetX, order.targetY)",
    "autoReturn: null",
    "autoReturn: { x: unit.x, y: unit.y }",
    "function canContinueAutomaticAttackTarget",
    "Math.hypot(target.x - unit.x, target.y - unit.y) <= reactionRange + target.radius",
    "sourceUnitHasGoal(target, unit.id) && isInAttackRange(target, unit, world)",
    "function restoreAutomaticAttackReturn",
    'kind: "attack-move"',
    "function stepHoldPositionOrder",
  ...
  ["save-game attack order", saveSource, [
    ...
    "order.autoReturn === null && !liveUnitIds.has(order.targetId)",
    "if (order.autoReturn)",
