...
const CANDIDATE_MAPS = (process.env.WARGUS_BROWSER_SPELL_MAPS ?? process.env.WARGUS_BROWSER_SPELL_MAP ?? [
  "campaigns/human-exp/levelx01h.smp.gz",
  "campaigns/human-exp/levelx10h.smp.gz",
  "campaigns/human-exp/levelx11h.smp.gz",
  "campaigns/human/level14h.smp.gz",
  "campaigns/orc-exp/levelx12o.smp.gz",
  "campaigns/orc/level06o.smp.gz"
].join(",")).split(",").map((map) => map.trim()).filter(Boolean);
const chromeProfile = mkdtempSync(path.join(tmpdir(), "wargus-spell-chrome-"));
...
try {
  ...
  await waitForExpression(client, "typeof window.__WARGUS_TS_LOAD_MAP__ === \"function\" && typeof window.__WARGUS_TS_ISSUE_FIRST_SPELL__ === \"function\" && typeof window.__WARGUS_TS_SAVE_ACTIVE_WORLD_ROUNDTRIP__ === \"function\"", 20_000);

  const failures = [];
  let verified = false;
  for (const mapPath of CANDIDATE_MAPS) {
    try {
      const result = await verifySpellMap(client, mapPath);
      if (pageErrors.length > 0) {
        throw new Error(`Browser page exceptions: ${pageErrors.join("; ")}`);
      }
      console.log(`Browser spell session verified (${mapPath}, spell=${result.spellId}, command=${result.command}, mana=${result.beforeMana}->${result.afterMana}, effects=${result.beforeEffects}->${result.afterEffects}, tick=${result.tick}).`);
      verified = true;
      break;
    } catch (error) {
  ...
  if (!verified) {
    throw new Error(`Unable to verify browser spell session on candidate maps:\n${failures.join("\n")}`);
  }
} finally {
  client?.close();
...
async function verifySpellMap(client, mapPath) {
  ...
  await waitForExpression(client, "Boolean(window.__WARGUS_TS_SMOKE_STATE__?.firstSpellCasterWorldPoint && window.__WARGUS_TS_SMOKE_STATE__?.firstSpellTargetWorldPoint && window.__WARGUS_TS_SMOKE_STATE__?.firstSpellCommand && window.__WARGUS_TS_SMOKE_STATE__?.firstSpellId)", 10_000);
  const before = await readSmokeState(client);
  if (!before.firstSpellCommand || !before.firstSpellId || !Number.isFinite(before.firstSpellCasterMana)) {
    throw new Error(`invalid spell pair before cast: ${JSON.stringify(before)}`);
  }
  const issued = await evalValue(client, "window.__WARGUS_TS_ISSUE_FIRST_SPELL__()");
  if (issued !== true) {
    throw new Error(`unable to issue first spell order: ${JSON.stringify(await readSmokeState(client))}`);
  }
  const castState = await waitForSpellProgress(client, before, 45_000);
  const save = await evalValue(client, "window.__WARGUS_TS_SAVE_ACTIVE_WORLD_ROUNDTRIP__()");
  if (!save?.ok || save.saveRoundtripOk !== true || !Number.isFinite(save.tick) || save.tick < castState.tick) {
    throw new Error(`spell active-world save/load roundtrip failed: ${JSON.stringify(save)}`);
  }
  return {
    spellId: before.firstSpellId,
    command: before.firstSpellCommand,
    beforeMana: before.firstSpellCasterMana,
    afterMana: castState.firstSelectedMana,
    beforeEffects: before.spellEffectCount,
    afterEffects: castState.spellEffectCount,
    tick: castState.tick
  };
}

async function waitForSpellProgress(client, before, timeoutMs) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    const state = await readSmokeState(client);
    const firstSelectedMana = state.firstSelectedMana ?? state.firstSpellCasterMana;
    if (
      state.spellEffectCount > before.spellEffectCount
      || (Number.isFinite(firstSelectedMana) && firstSelectedMana < before.firstSpellCasterMana)
      || state.firstSelectedOrderKind === "spell-cast"
    ) {
      return { ...state, firstSelectedMana };
    }
    await delay(500);
  }
  throw new Error(`timed out waiting for spell progress; before=${JSON.stringify(before)}, smoke=${JSON.stringify(await readSmokeState(client))}`);
}
