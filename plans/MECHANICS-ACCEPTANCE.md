...
| M08 | 014 | `ai-staged-pressure`; normal difficulty | Observe source land AI from opening through first three launches | Default zero-cycle sleep advances; 1-unit launch detaches before 4-unit wait, then 4 before 16; launched ids are not reused; Hall is not orphaned/overcommitted | Script index, force declaration/assigned/launched ids, attack-order timeline, desired/completed buildings, reserved resources |
...
| M13 | 017 | `pace-a`, `pace-b`, `pace-c`; default difficulty; land-AI enemy source slots only | Run economy-first and pressure-first openings | Honest selected pace, no unit-speed mutation or AI-strategy confound, Hall/first unit/contact inside accepted windows across seeds | Milestone table, source player ids/enemy AI type, start distance, telemetry budgets, final recommendation |
